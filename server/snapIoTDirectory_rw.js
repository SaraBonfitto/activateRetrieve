...
router.route('/amqp')
 .post(function(req, res) {
    console.log("entrato amqp");
		...
		var args = [];
		if(registeredStub.indexOf(req.body.contextbroker)>0)
		{
			res.json({ message: 'stub already active for ORION context broker ' + req.body.contextbroker});
		}
		else
...
router.route('/killprocess')
 .get(function(req, res) {
	 var toKill = registeredStub[req.body.contextbroker];
	 console.log("registeredStud "+ registeredStub[req.body.contextbroker]);

	 toKill.stdin.write("kill");
//	toKill.kill();
	 delete registeredStub[toKill];
 
 	res.json({ message: registeredStub});
});
